...
def main():
    ...
    if args.task in ['selfapr']:
        tokenizer.add_tokens(
            ['{', '}', '<', '^', '<=', '>=', '==', '!=', '<<', '>>', '[PATCH]', '[BUG]', '[CE]', '[FE]', '[CONTEXT]',
             '[BUGGY]', '[CLASS]', '[METHOD]', '[RETURN_TYPE]', '[VARIABLES]', '[Delete]'])
        model.resize_token_embeddings(len(tokenizer))
    elif 'selfapr_rapgen' in args.task:
        tokenizer.add_tokens(
            ['{', '}', '<', '^', '<=', '>=', '==', '!=', '<<', '>>', '[PATCH]', '[BUG]', '[EXAMPLE_BUG]', '[CE]',
             '[FE]', '[CONTEXT]',
             '[BUGGY]', '[CLASS]', '[METHOD]', '[RETURN_TYPE]', '[VARIABLES]', '[Delete]'])
        model.resize_token_embeddings(len(tokenizer))
    ...
    if args.do_test:
        ...
        if 'selfapr' not in args.task:
            ...
            # Generate predictions for Defects4J bugs
            ...
            model.load_state_dict(torch.load(file))
            split_tag = 'test'
            eval_examples, eval_data = load_and_cache_gen_data(args, args.test_filename, pool, tokenizer, split_tag,
                                                               only_src=True, is_sample=False)
            logger.info("  ***** Running bleu evaluation on {} data*****".format(split_tag))
            logger.info("  Num examples = %d", len(eval_examples))
            logger.info("  Batch size = %d", args.eval_batch_size)
            eval_sampler = SequentialSampler(eval_data)
            eval_dataloader = DataLoader(eval_data, sampler=eval_sampler, batch_size=args.eval_batch_size)
            # Defects4j test data, obtained from SelfAPR repo:
            # https://github.com/ASSERT-KTH/SelfAPR/tree/monperrus-patch-1/dataset/test.csv
            d4j_test_file = '{}/{}/{}'.format(args.data_dir, 'selfapr', 'defects4j.csv')
            df_test = pd.read_csv(d4j_test_file, delimiter='\t')
            assert len(df_test) == len(eval_dataloader)
            model.eval()
            cnt = 0
            d4j_file = 'Defects4J_' + args.task + '_beam_100'
            d4j_repair_dir = os.path.join(args.output_dir, d4j_file)
            if not os.path.exists(d4j_repair_dir):
                os.makedirs(d4j_repair_dir)
            for batch in tqdm(eval_dataloader, total=len(eval_dataloader),
                ...
                with torch.no_grad():
                    ...
                    pred_nls = [tokenizer.decode(g, skip_special_tokens=True, clean_up_tokenization_spaces=True) for g in
                             generated_ids]
                d4j_id = str(cnt) + '-' + df_test['id'][cnt]
                output_fn = os.path.join(d4j_repair_dir, "test_{}.output".format(str(d4j_id)))
